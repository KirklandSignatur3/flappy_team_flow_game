...
public class TeamFlowPipeTurnsState extends State{
    private Random rand;
...
//    private float BGChangeFrequency = BGChangeTimeBase + rand.nextFloat(BGChangeTimeFluctuation);
    ...
    public TeamFlowPipeTurnsState(GameStateManager gsm) {
        super(gsm);
...
//        font.setColor(0f, 0f, 0f, 1.0f);
        ...
        log = new Log("C:/Users/maxwe/Documents/SCHOOL/SURF/Team flow/Gameplay data/","TeamFlowPipeTurnsState");

        pipes = new ArrayList<Pipe>();
        pipes.add(new Pipe(START_GAP, 130, 140, 90, 50));
        for(int i = 1; i<PIPE_COUNT; i++){ //original gap 140
            pipes.add(new Pipe(START_GAP+ i* (PIPE_SPACING + Pipe.PIPE_WIDTH ), 130, 140, 90, 50));
        }
...
//            newscore_str = "   + " + popupscore;
        ...
        ScoreStr = "Points: " + score +"  ";
        font.draw(sb, "Time: "+String.format("%.1f", time), player.getPosition().x-260, 395); //-110
//        System.out.println((jump_time_window - jump_time));
//        font.draw(sb, ScoreStr+ " Streak: " + streak, player.getPosition().x-250, 395);
        font.draw(sb, ScoreStr+ " Streak: " + streak, player.getPosition().x-40, 395);
